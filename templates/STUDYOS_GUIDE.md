...
- `analysis/` contains import plans, inventory, batch plans, source digests, learning cores, visual notes, validation records, and processing state.
...
- `studyos-import` proposes and executes safe import, then creates inventory and the first conceptual batch plan.
...
2. Run `studyos-import` in proposal mode.
3. Run `studyos-import` in execute mode after approving the import plan.
4. Run `studyos-plan`.
5. Run `studyos-batch` for one batch.
6. Run `studyos-validate`.
7. Run `studyos-course` if you want remaining batches processed sequentially.
8. Run `studyos-merge` when course outputs are processed and validated.
...
- Missing import plan: run `studyos-import` in proposal mode.
- Empty `inputs/`: run `studyos-import`.
- Missing `batch_plan.md`: run `studyos-import`, then `studyos-plan`.
